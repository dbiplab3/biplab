...
function resolveAsync(rawTaskOptions, rawTargetOptions, targetName, resolvedFiles, theTemplateProcessor, theGlobExpander, theVerboseLogger) {
    ...
    return new es6_promise_1.Promise(function (resolve, reject) {
        ...
        (0, visualStudioOptionsResolver_1.resolveVSOptionsAsync)(result, rawTaskOptions, rawTargetOptions, templateProcessor).then(function (result) {
            (0, tsconfig_1.resolveAsync)(result, rawTaskOptions, rawTargetOptions, templateProcessor, globExpander, verboseLogger).then(function (result) {
...
function resolveAndWarnOnConfigurationIssues(task, target, targetName) {
    ...
    checkFixableCaseIssues(target, "target \"".concat(targetName, "\""));
    checkLocations(task, 'ts task');
    checkLocations(target, "target \"".concat(targetName, "\""));
    warnOnFilesUsedWithFast(task, 'ts task');
    warnOnFilesUsedWithFast(target, "target \"".concat(targetName, "\""));
    ...
    function getAdditionalWarnings(task, target, targetName) {
        ...
        if (propertiesFromTarget.indexOf(targetName) >= 0) {
            additionalWarnings.push("Warning: Using the grunt-ts keyword \"".concat(targetName, "\" as a target name may cause ") +
                "incorrect behavior or errors.");
        }
        if (((task && task.src && targetName !== 'src') || (target && target.src)) &&
            ((task && task.files) || (target && target.files))) {
            additionalWarnings.push("Warning: In task \"".concat(targetName, "\", either \"files\" or \"src\" should be used - not both."));
        }
        if (((task && task.vs) || (target && target.vs)) &&
            ((task && task.files) || (target && target.files))) {
            additionalWarnings.push("Warning: In task \"".concat(targetName, "\", either \"files\" or \"vs\" should be used - not both."));
        }
        if (usingDestArray(task) || usingDestArray(target)) {
            additionalWarnings.push("Warning: target \"".concat(targetName, "\" has an array specified for the files.dest property.") +
                "  This is not supported.  Taking first element and ignoring the rest.");
        }
        if ((task && task.outFile) || (target && target.outFile)) {
            additionalWarnings.push("Warning: target \"".concat(targetName, "\" is using \"outFile\".  This is not supported by") +
    ...
    function warnOnFilesUsedWithFast(task, configName) {
        if (task && task.files && task.options && task.options.fast) {
            warnings.push("Warning: ".concat(configName, " is attempting to use fast compilation with \"files\".  ") +
            ...
            for (var propertyName in task) {
                if (propertiesFromTarget.indexOf(propertyName) === -1 && propertyName !== 'options') {
                    if (propertiesFromTargetOptions.indexOf(propertyName) > -1 &&
                        ...
                        var warningText = "Property \"".concat(propertyName, "\" in ").concat(configName, " is possibly in the wrong place and will be ignored.  ") +
                        ...
                        var warningText = "Property \"".concat(propertyName, "\" in ").concat(configName, " is possibly in the wrong place and will be ignored.  ") +
                            "It is expected on the options object.  It is also the wrong case and should be ".concat(correctPropertyName, ".");
                        warnings.push(warningText);
                    }
                }
            }
                ...
                for (var propertyName in task.options) {
                    if (propertiesFromTargetOptions.indexOf(propertyName) === -1) {
                        if (propertiesFromTarget.indexOf(propertyName) > -1) {
                            ...
                            var warningText = "Property \"".concat(propertyName, "\" in ").concat(configName, " is possibly in the wrong place and will be ignored.  ") +
                                "It is expected on the task or target, not under options.  It is also the wrong case and should be ".concat(correctPropertyName, ".");
                            warnings.push(warningText);
                        }
                    }
                }
        ...
        if (task) {
            for (var propertyName in task) {
                if ((propertiesFromTarget.indexOf(propertyName) === -1)
                    ...
                    var index = lowercaseTargetProps.indexOf(propertyName.toLocaleLowerCase());
                    var correctPropertyName = propertiesFromTarget[index];
                    var warningText = "Property \"".concat(propertyName, "\" in ").concat(configName, " is incorrectly cased; it should ") +
                        "be \"".concat(correctPropertyName, "\".  Fixing it for you and proceeding.");
                    warnings.push(warningText);
            ...
            for (var propertyName in task.options) {
                if ((propertiesFromTargetOptions.indexOf(propertyName) === -1)
                    ...
                    var index = lowercaseTargetOptionsProps.indexOf(propertyName.toLocaleLowerCase());
                    var correctPropertyName = propertiesFromTargetOptions[index];
                    var warningText = "Property \"".concat(propertyName, "\" in ").concat(configName, " options is incorrectly cased; it should ") +
                        "be \"".concat(correctPropertyName, "\".  Fixing it for you and proceeding.");
                    warnings.push(warningText);
...
function addressAssociatedOptionsAndResolveConflicts(options) {
    ...
    if ('comments' in options && 'removeComments' in options) {
        options.warnings.push("WARNING: Option \"comments\" and \"removeComments\" should not be used together.  " +
            "The --removeComments value of ".concat(!!options.removeComments, " supercedes the --comments value of ").concat(!!options.comments));
    }
    if ('comments' in options && !('removeComments' in options)) {
