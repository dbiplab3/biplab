...
function quotedRelativePath(thePath, relativeTo) {
    if (relativeTo === void 0) { relativeTo = '.'; }
    return "\"".concat(stripQuotesIfQuoted(path.relative(relativeTo, path.resolve(thePath))), "\"");
}
...
function readAndParseJSONFromFileSync(fileName, encoding) {
    ...
    try {
        ...
        throw new Error("Error reading file ".concat(fileName, ": ").concat(ex));
    }
    try {
        result = JSON.parse(textContent);
    }
    catch (ex) {
        throw new Error("Error parsing JSON in file ".concat(fileName, ": ").concat(ex));
    }
    return result;
}
