...
function compileAllFiles(options, compilationInfo) {
    ...
    if (compilationInfo.outDir && options.baseDir && files.length > 0 && !options.rootDir) {
        ...
        if (!fs.existsSync(baseDirFilePath)) {
            var baseDir_Message = "// grunt-ts creates this file to help TypeScript find " +
                ...
                "it, specify a `rootDir` setting in the ".concat(settingsSource, ".  See ") +
        ...
        if (tscVersion && semver.satisfies(tscVersion, '>=1.5.0')) {
            exports.grunt.log.warn(("Warning: created ".concat(baseDirFilePath, " file because `outDir` was ") +
                "specified in the ".concat(settingsSource, ", but not `rootDir`.  Add `rootDir` ") +
                " ".concat(settingsSection, " to fix this warning.")).magenta);
        }
    ...
    exports.grunt.log.verbose.writeln("TypeScript path: ".concat(tsc));
    if (tsconfig && tsconfig.passThrough) {
        ...
        if (options.lib) {
            ...
            options.lib.forEach(function (option) {
                if (possibleOptions_1.indexOf((option + '').toLocaleLowerCase()) === -1) {
                    exports.grunt.log.warn("WARNING: Option \"lib\" does not support ".concat(option, " ").magenta);
                }
            });
        ...
        if (options.types) {
            args.push('--types', "\"".concat(_.map(options.types, function (t) { return utils.stripQuotesIfQuoted(t.trim()); }).join(','), "\""));
        }
        if (options.typeRoots) {
            args.push('--typeRoots', "\"".concat(_.map(options.typeRoots, function (t) { return utils.stripQuotesIfQuoted(t.trim()); }).join(','), "\""));
        }
